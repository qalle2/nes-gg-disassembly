...
color_multiuse2 equ $20  ; hand cursor 2, revolv. cursor 2, particles 2 (white)
...
tile_to_vrambuf ; Copy one nybble/tile of a graphic to VRAM buffer.
                ...
                and  #%00000001       ; if even offset, upper nybble;
                beq  +                ;  if odd, lower
                lda  temp1
                and  #%00001111
                jmp  ++
...
--              cpy vram_buf_wr_pos  ; exit if all blocks copied
                ...
                add #5
                sta vram_block_cost
                lda vram_budget      ; remove cost from budget; exit if out of
                sub vram_block_cost  ; budget
                bcc +
                sta vram_budget
...
init_loop_x     ; copy one row of graphics data to target sprites
                ...
                ; push high nybble if even offset, else low
                lda nybble_offs
                and #%00000001
                beq +
                lda temp1
...
update_attr_blk ; Change the value of an attribute block (2 bits) within one
                ...
                ; vram_attrtable0 + A -> VRAM block target address
                ora #<vram_attrtable0
                sta vram_block+2
                lda #>vram_attrtable0
                sta vram_block+1
...
-               lda keybd_graphic    ; draw letter (A=id, X/Y=position)
                ...
                sta keybd_graphic_y
                ;
+               ; loop until last letter (id 18 = "N") has been drawn
                inc keybd_graphic
                lda keybd_graphic
                cmp #(18+1)
                bne -
...
+               inc keybd_graphic    ; loop until last dash has been drawn
                ...
                ; "Game Genie" logo (A=byte, X=rows, Y=first row)
                lda #%01010101
                ldx #4
                ldy #0
                jsr fill_atrblkrows
                ...
                ; highlight top left letter on virtual keyboard
                ldx #0
                ldy #0
                jsr hilite_atr_byte
...
+               ldx #hand_y_pixel       ; no terminator
                ; A = speed, X = address of position; out: Z
                jsr move_hand2
                ; every 2nd frame, skip rest of vertical movement
                beq move_hand_exit
...
upd_hand_ltrpos ; Update letter position of hand cursor. Called by move_hand.
                ...
                ldy hand_y_keyboard
                jsr clear_atr_byte
                ; where's the target letter (virtual keyboard or input area)?
                lda hand_y_letr_trg
                cmp #2
                bcs +
...
init_sprite_atr ; Initial sprite attribute data. Bytes are in reverse order.
                ...
                ; Read by init3.
                ; 19-15
                db %00011001, %00011001, %00011001, %00011001, %00011010
                ; 14-10
                db %00011001, %00011001, %00011001, %00011001, %00011010
                ; 9-5
                db %00011001, %00011001, %00011001, %00011001, %00011010
...
letter_input    ; Button A went from off to on and hand cursor is on virtual
                ...
                pla
                pha
                jsr draw_bg_graphic  ; draw graphic (A = id, X/Y = X/Y pos)
                jsr input_effects
                pla                  ; pull id
                jsr save_letter
...
+               inc revolv_x_letr1   ; may be undone later
                ...
                ; if 3rd one is A/P/Z/L/G/I/T/Y, code is complete
                lda revolv_y_letr1
                asl a
                asl a
                asl a
...
+               lda hand_y_letr
                cmp #2
                bcs +
                jmp letter_input     ; hand on keyboard   (sub ends with RTS)
+               jmp move_revolv_man  ; hand on input area (sub ends with RTS)
...
-               lda init_prt_spds_x,y  ; horizontal position
                ...
                lda #%00011010         ; attribute (unused %110, palette 2)
                sta sprite_data_atr,x
                inx                    ; end of loop
                iny
                cpy #16
...
move_parti_set2 ; process 2nd set of particles (from every 2nd explosion)
                lda parti_set2timer
                beq +
                ldx #48
                inc parti_set2timer
...
++              sta sprite_data_y,x
                ...
                ; slow particle down every 8th frame (temp1 = timer mod 8)
                lda temp1
                bne particle_moved
                lda parti_spds_x,x
...
dec_abs_value   ; If number is nonzero, decrement absolute value in 2's
                ; complement.
                ; In: A = number to decrement; negative/zero flags must reflect
                ;         it.
...
fix_revolv_x    ; Move revolving cursor left until it lies immediately after a
                ...
                lda entered_letrs,x
                bne ++
-               ; move cursor left until at column 0 or a letter
                dex
                lda entered_letrs,x
                bne ++
                dec revolv_x_letr1
...
hilite_inp_row  ; Highlight active row on input area using Attribute Table.
                ...
                ; exit if Y position of revolving cursor unchanged
                lda revolv_y_ltr2pr
                cmp revolv_y_letr2
                beq hilite_exit
...
check_sel_start ; If select or start pressed, decode entered codes and start
                ; game.
                ; Called by do_every_frame.
...
-               sta decoded_codes,x
                ...
                ; OR bitmask to enable compare value of 1st code in
                ; genie_ctrl_val
                lda #%00000010
                sta comp_enab_mask
...
decodeallcodes  ; start the long outer loop
...
-               lda (code_ptr),y
                ...
                and #%00001111
                lsr a
                ; bit 3 = least significant bit of previous letter
                ora temp2
                sta (code_ptr),y
                ; the bit that was just shifted out -> bit 3 of temp2
                lda #$00
                rol a
                asl a
                asl a
...
+               ; if code is not 6 or 8 letters, ignore it
                sty code_length
                cpy #8
                beq +
                cpy #6
...
+               ; regardless of code length, read 4th value and discard it
                ldy #3
                lda (code_ptr),y
                jmp +++
++              ldy #3
...
+++             ; copy the bit that was shifted out from the last value,
                ...
                ; code to 4 bytes in decoded_code according to codekey.
...
next_code       ; prepare for next code by shifting bitmasks left
                sec
                rol code_enab_mask
                asl comp_enab_mask

                ; advance source pointer (high byte never increments)
                lda code_ptr+0
                add #8
                sta code_ptr+0
                bcc +
...
+               ; advance target pointer (high byte never increments)
                lda decoded_cod_ptr+0
                add #4
                sta decoded_cod_ptr+0
                bcc +
...
codekey         ; how to descramble codes; read by check_sel_start
                db 3, 5, 2, 4, 1, 0, 7, 6
