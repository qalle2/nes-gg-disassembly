...
; for overriding the Ophis assembler's automatic zero page optimization
.macro dec_absolute
    .byte $ce, _1, $00
.macend
.macro lda_absolute
...
ram_clear_loop:
    ...
    `dec_absolute vram_buffer_read_pos2
...
draw_graphic_on_background:
    ...
    `lda_absolute gfx_ptr
    clc
    adc #2
    `sta_absolute gfx_ptr
    `lda_absolute gfx_ptr+1
    adc #0
    `sta_absolute gfx_ptr+1
...
clear_loop:
    ...
    lda #14
    `sta_absolute hand_x_px
    sta metasprite_x
    lda #60
    `sta_absolute hand_y_px
    sta metasprite_y
...
do_every_frame:
    ...
    `lda_absolute hand_x_px
    sta metasprite_x
    `lda_absolute hand_y_px
    sta metasprite_y
...
check_arrows:
    ...
    `lda_absolute hand_y_speed_ptr+1
    ...
    `sta_absolute hand_y_speed_ptr
    lda #>hand_speeds_negative
    `sta_absolute hand_y_speed_ptr+1
...
check_horizontal_arrows:
    ...
    `lda_absolute hand_x_speed_ptr+1
    ...
    lda #<hand_speeds_negative
    `sta_absolute hand_x_speed_ptr
    lda #>hand_speeds_negative
    `sta_absolute hand_x_speed_ptr+1
...
change_revolving_cursor_attributes:
    ; change attributes of sprites 20-23
    lda sprite_attributes+20
    cmp #%00011010  ; VHBUUUPP
    bne use_3rd_subpalette
    lda #%00011000
    jmp subpalette_chosen
use_3rd_subpalette:
    lda #%00011010
subpalette_chosen:
    ...
    ora #%00100000  ; put behind background if phase is 0-6
...
sub2:
    ...
    ; compute X speed for revolving cursor
    lda revolving_x
    ...
    ; compute Y speed for revolving cursor
    lda revolving_y
    `sta_absolute revolving_pos
    ...
    ; increment phase of revolving cursor
    inx
    cpx #16
...
phase_incremented:
    ...
    jmp change_revolving_cursor_attributes  ; ends with rts
